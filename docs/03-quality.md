...
- `.github/workflows/release.yml` 负责 release artifact 构建、`SHA256SUMS` 生成和 GitHub release asset 发布；配置 GitHub repository secret `GITEA_TOKEN` 后，也可直接发布到 Gitea release。npm publish job 在 Rust 平台产物就绪后运行，先发布 `@mars167/core-*` 平台包，再发布主包 `@mars167/codetrail`。预发布版本使用 npm dist-tag `next`，稳定版本使用 `latest`。
