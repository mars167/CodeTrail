...
- `call-hierarchy <identifier>` 查询 Java incomingCalls/outgoingCalls 结构化调用层级，默认 `--depth 2`。它需要 fresh Java semantic index；缺失时返回空结果和 freshness 说明。公开层级只返回已解析的 callable 节点并显示签名；方法和构造器会显示，没解析到唯一签名的裸调用点不作为 hierarchy function 返回。Text 输出采用 `Class.method(args)  (package)` 的紧凑形状。
