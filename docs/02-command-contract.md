...
导航命令还支持由前序结果给出的 symbol coordinate 输入：

```text
<workspace-relative-path>:<line>#<symbol-name-or-qualified-name>
```

`defs`、`symbols`、`refs`、`calls`、`callers` 和 `call-hierarchy`
会先按 coordinate 的 path、line 和 symbol 限定候选；没有命中时再回退到
`--input-mode compatible` 的普通符号展开。调用方应优先复制结果里的
`queryInputs.defs` / `queryInputs.symbols` / `queryInputs.calls` /
`queryInputs.callers` / `queryInputs.callHierarchy`，不要自行从显示文本拼接。
...
- symbol-like 结果、route handler、调用目标和 call-hierarchy 节点可包含 `queryInputs`。这些字段是公开导航契约，值可以原样传给后续 `defs`、`symbols`、`calls`、`callers` 或 `call-hierarchy`。
...
- `symbols/defs` 默认结果行包含 kind、name、位置和可用的 `defs=<coordinate>`，保持一行可跳转且可串联。
- `routes` 在能解析 handler 定义时追加 `defs=<coordinate>`；`calls`/`callers` 在能解析目标定义时追加 `target=<coordinate>`；`call-hierarchy` 节点在能解析定义时追加 `defs=<coordinate>`。
